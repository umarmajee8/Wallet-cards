// patch24 source: the Settings sheet after round 12 - Stack and Carousel as two independent
// configuration modes, a preview that always stacks at least three real cards, and sliders that
// glide the wallet instead of snapping it.
//
// Readable form of the function patch24 splices into app/index.js. patch24 deletes these `//`
...
pv=(()=>{let want=isStack?6:3,l=(cs||[]).filter(z=>z&&z.src).slice(0,want),pal=[`#2c3d56`,`#5a3a2c`,`#1f4436`,`#4b3b6b`,`#6b1f2a`],tt=l[0]&&l[0].title||``;
for(let i=l.length;i<want;i++){let cl=pal[i%pal.length];l.push({id:`__cwph${i}`,title:tt,subtitle:``,fields:[],color:cl,
src:`data:image/svg+xml,`+encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="760" height="494"><rect width="760" height="494" fill="')+cl+'"/></svg>'})}
return l})(),
